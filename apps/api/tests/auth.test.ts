...
describe('GET /api/auth/config', () => {
  it('returns feature flags with defaults off', async () => {
    jest.replaceProperty(features, 'REQUIRE_EMAIL_VERIFICATION', false)
    jest.replaceProperty(features, 'ENABLE_PASSWORD_RESET', false)
  ...
  it('reflects flags when both are enabled', async () => {
    jest.replaceProperty(features, 'REQUIRE_EMAIL_VERIFICATION', true)
    jest.replaceProperty(features, 'ENABLE_PASSWORD_RESET', true)
...
describe('POST /api/auth/register', () => {
  ...
  it('creates account and returns 201 when verification is off', async () => {
    ...
    db.user.findUnique.mockResolvedValue(null) // no existing email or nickname
  ...
  it('returns 409 when nickname is already taken', async () => {
    db.user.findUnique
        ...
        .mockResolvedValueOnce(mockUser1 as unknown as User)  // nickname taken
